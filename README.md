...
- configurable inventory groups with editable cargo, weapon, power, refinery, assembler, gas, tool, safety, connector and safe unknown-definition presets; each group's editor lists include and exclusion rules with buffered Apply/Cancel and Ctrl/Shift multi-selection;
...
To exclude a modded block such as a refinery-based shield generator, open **Groups → select Refineries → Edit → Add exclusion**, choose its **Block definition**, then **Save rule → Apply**. Leave roles and items unrestricted to exclude the whole definition, including future blocks of that definition on this ship. Exclusions override all include rules in that group. They can also select specific blocks or named terminal groups, or restrict a role or material. They affect only the edited group and actions/loadouts using it; other groups and ship-wide automation remain independent. A group with no include rules matches nothing. Removing the exclusion restores the original inclusion behavior.

Default layouts keep ore, ingots, components, ammunition, tools and other item categories together, preserving remembered positions within each category. Existing remembered layouts are grouped on upgrade; a subsequent drag opts that section into a fully custom order. Rebalance and local Drain jobs stay unobtrusive for the first two seconds. Longer jobs show a compact cancellable progress window that closes on success; failures remain inspectable. Closing the terminal stops remaining local rebalance and drain requests, including during the initial hidden period.

### Inventory controls

Double-click or Enter transfers a stack to the opposite column. Alt-click transfers one item, Ctrl-click transfers 10 on release, Shift-click 100, and Ctrl+Shift-click 1,000 (capped by the available amount). Alt-click is built in while Unified Storage controls the inventory page, including mixed columns; Grab Single Item is not required. Holding Alt suppresses dragging and double-click stack transfers. Ctrl-drag or right-drag opens the amount dialog; ordinary left-drag moves a stack. Holding Ctrl when dropping also opens the amount dialog. Dragging within a unified section changes its remembered display order, not physical stack splitting. Refinery input order remains controlled by ore priority. Native item-use actions and character-item dropping are available too.
