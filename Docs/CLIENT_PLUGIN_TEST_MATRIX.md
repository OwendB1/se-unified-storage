...
2. Re-enable Unified. Exercise All, Energy, Ship, Storage, System, search, Hide Empty, scrolling, mouse dragging, right-click amount entry, double-click, and gamepad A transfer.
   Alt-click regression (pending live verification): with and without Grab Single Item loaded, hold either Alt key and left-click a stack. Verify exactly one item transfers per press, or the remaining amount for ore/ingots below one. Repeat rapidly as a double-click and move the mouse while held: no full-stack transfer, drag or reorder may occur. Test character-to-unified, unified-to-character, unified-to-unified and both mixed-column directions, in a local world and on SDX2. With Alt held, Ctrl/Shift/Ctrl+Shift must retain 10/100/1,000 limits. Without Alt, verify plain click, double-click, Ctrl-click on release, Ctrl-drag and right-drag remain functional. Turn both Unified toggles off and verify Grab Single Item still handles the vanilla page when loaded.
...
## Group exclusions — pending live acceptance

- Edit the Refineries preset, Add exclusion, and select a refinery-based shield generator's Block definition. Save is disabled until a selection is made. Leave All roles and All items selected, Save rule, then Apply. Only ordinary refineries remain in that group; a second group including the shield definition remains unchanged. New shield generators of that definition are also excluded. Remove the exclusion and confirm membership returns.
- Cancel the child rule editor and the parent group editor separately; neither commits drafts. Duplicate an exclusion, edit its action, reopen the group, and restart the game to verify independent copies and persistence. Check the Action column, long definition names and both button rows at the active resolution.
- With overlapping include rules, exclude a specific block or named terminal group. Exclusions win regardless of row order. Restrict an exclusion to Production input or one ore; other roles/materials remain accessible. An exclusion-only group stays empty. A missing terminal-group selector pauses the whole group; restoring its name recovers it.
- Exercise display, drag/keyboard transfers, rebalance and loadout target/supply/return through the edited group on client and companion. Excluded members/items must never participate through it. Apply an exclusion while work is queued; the group guard stops remaining work. Ship-wide refinery sorting and other groups remain independent.
- Load schema 1 and 2 profiles, then save/reload schema 3. Publish/fetch/adopt exclusions with an updated companion. A companion advertising GroupRules but not GroupExclusions must retain ownership coordination while using standalone transfers and disabling profile exchange.
