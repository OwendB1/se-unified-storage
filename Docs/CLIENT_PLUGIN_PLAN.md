...
Each ship-local profile stores an ordered list of groups with stable IDs, display names and a list of rules. Each rule has a block selector, optional inventory-role filter, and optional item-category/exact-item filters. Select blocks by all blocks, known family, object-builder type, exact block definition, terminal block-group name, specific block entity, or a loaded production blueprint's result item. Conditions within a row are AND; rows are alternatives (OR). Recipe-output selection identifies production blocks capable of that output; their input/output role and material filters remain separate choices.

Editing a group opens its name and a large rules table (match by, definition/type, inventory role, items). Add/Edit opens the individual rule form; Save rule returns to the draft list. Duplicate/Remove support Ctrl/Shift multi-selection; Edit requires one row. The group's Apply commits its name and all draft row changes together; Cancel, Escape and X discard the entire draft, including saved child edits. An explicitly empty rule list matches nothing. Existing schema-1 groups migrate to one-rule schema-2 groups without changing IDs, selectors, filters or loadout references; preset groups start with one family rule.

Rules retain their selector/role/item association through projection, deposits, rebalance, and loadout target/supply/return resolution. For example, `Guns group + Ammunition + Magazine A` OR `Reactors group + Fuel + Uranium` must never admit uranium into the Guns branch merely because another row accepts it. Overlapping rules count each physical inventory and stack once. A missing named terminal group pauses the whole group, not just that row, so maintenance cannot redistribute stock across an incomplete group. Membership guards include every rule's definition and resolved membership.

The optional companion advertises `GroupRules` support. Without it, updated clients retain ownership coordination but use standalone inventory transfers and disable companion actions/profile exchange, preventing an older server from ignoring the rule list. Update both ends before sharing schema-2 profiles. No companion is required for the rules editor or its local behavior.
...
- Manage uses friendly state and checkbox labels: no exclusions is **Managed**, ManualBlock is **Unmanaged**, ReservedInventory is **Reserved**, and NoUnifiedCargoDestination is **Source Only**. Combined flags show all applicable names, and unsaved asterisks remain. Tooltips explain the restrictions; serialized flags and behavior are unchanged.
- List dialogs support Ctrl-click toggles, anchored Shift-click ranges, additive Ctrl+Shift ranges and Ctrl+A over filtered results, using native multi-select rendering. Manage checkbox edits stage changes for all selected members, with mixed-state indicators and cargo-only destination flags; Apply retains selection. Groups support batch duplicate/delete/reorder; loadouts support batch delete; their detailed editors require exactly one selected row. Ore priorities support batch pin/unpin and relative-order-preserving moves. Component targets support batch quantity changes while retaining each recipe; editing a recipe requires one row, and blank mixed targets cannot silently clear a batch. Craft deficits and Apply loadouts remain scope-wide actions. Refresh/rebuild restores surviving selections by identity, not stale row positions.
- Present scope choices as a ship/network tree: selectable ship parents mean whole construct, indented network (and optional group) children retain stable IDs and accessed markers. Keep parent before children and the accessed construct first. Draw `├─` / `└─` branches in the native text indentation gutter without changing the font, using native row positions, clipping and highlight colors. This also covers the closed selection and scrolled/flipped dropdown. Ship names use available width, without a character-count cutoff; native fitting/ellipsis handles actual overflow. Full ship details remain in tooltips; off columns keep the vanilla filtering behavior.
- Rebalance uses a modal progress screen for client-side batches: processed item-plan count, confirmed quantities, elapsed time, partial/failure summary and Cancel. Closing it or deactivating the terminal cancels only its remaining batch, while an in-flight request is still acknowledged; accepted transfers are never rolled back. Batch failures are summarized instead of emitting a notification for every item. Recreated section headers do not replay hover sounds; explicit click sounds remain. Server job monitors show elapsed time and confirmed progress, request cancellation on Close, and wait for acknowledgement; loss of contact explicitly leaves the outcome unknown. A bounded server action already submitted without a job ID cannot be cancelled retroactively.
- A column with Unified disabled has no scope dropdown or reserved dropdown space, and ignores the remembered unified scope filter. Native-style All lists connected constructs; Current ship lists the accessed mechanical construct, excluding connector-docked ships. Re-enabling Unified restores its saved dropdown choice.
- Separate types with vanilla-style dark inventory panels and the native list gap, not horizontal separator lines. Each panel contains its type title, action buttons and all inventory roles; the common toolbar and totals stay outside those panels. Backgrounds do not intercept input or change drag/drop ownership. Both refinery and assembler utility buttons read **Drain**; tooltips explain their different eligibility rules. Toggle glyph coordinates use a shared renderer-representable pixel grid so left/right frame thickness remains symmetric on ultrawide screens.
- Manage buffers exclusion edits across all selected rows until Apply saves the batch. Pending rows show an asterisk. Manual edits cover all inventories on the same block; Reserved and Not a cargo destination remain per-inventory. Close, X and Escape discard unapplied edits. Apply updates only edited flag bits, preserves other current settings, and keeps the selection and viewport. No automatic shared-profile publish.
- Provide context-specific tooltips on every plugin window caption and dialog/action button, including disabled actions and the close icon. Explain whether changes are buffered, saved immediately, or start transfers/production; job monitors explain close-to-cancel semantics, acknowledgement and unknown outcomes. Give fields and selectors scope/constraint explanations, and wrap longer help text for native tooltips. Save target explicitly applies to selected quantities; ore priorities still save immediately.
