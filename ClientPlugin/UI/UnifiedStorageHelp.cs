...
internal static class UnifiedStorageHelp
{
    ...
    public static string Screen(string caption) => Wrap(caption switch
    {
        ...
        "Edit inventory group" => "Include rules add inventories; matching exclusions take precedence in this group only. Each rule's block, role and item filters stay together; overlaps count once. Apply saves the draft.",
        "Edit group rule" => "All conditions in a rule must match. Exclusions affect only this inventory group. Terminal-group names automatically follow membership changes.",
    ...
    public static string Button(string caption, string text) => Wrap((caption, text) switch
    {
        ...
        ("Edit inventory group", "Apply") => "Maintained loadouts using this group use the new rules on their next pass. Without include rules, the group matches nothing.",
        ("Edit inventory group", "Add rule") => "Include rules are alternatives. Exclusions override them. Maximum 128 total rules per group.",
        ("Edit inventory group", "Add exclusion") => "Pick a block definition on this ship to exclude from this group, including future blocks of that definition. You can also select specific blocks or terminal groups. Other inventory groups and ship-wide automation are unaffected.",
    ...
    public static string Field(string label) => Wrap(label switch
    {
        ...
        "Rule action" => "Include adds matching inventories and items. Exclude removes matches from this group, even when another rule includes them. Leave roles and items unrestricted to exclude whole blocks.",
