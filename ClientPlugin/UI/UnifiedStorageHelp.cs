...
internal static class UnifiedStorageHelp
{
    ...
    public static string Screen(string caption) => Wrap(caption switch
    {
        ...
        "Edit inventory group" => "Edit the group's name and rule list. Any matching rule includes an inventory; each row's block, role and item filters stay together. Overlaps count once. Apply saves the entire draft; Cancel or X discards it, including edits made in child dialogs.",
        "Edit group rule" => "Edit one rule in the group's draft. All conditions in this row must match. Save rule returns to the list without saving the group; Apply in the parent editor commits all rows. Terminal-group names include future matching blocks automatically.",
        "Group actions" => "Open the available settings for this block type. Opening an editor does not itself move items.",
    ...
    {
        ("Unified Storage members", "Apply") => "Save all pending inventory exclusions, including edits on other rows. Keep this window and its selection open. Does not publish a shared profile.",
        ("Unified Storage members", "Close") => "Close and discard every unapplied row edit. Previously applied settings remain saved. Escape and the X button do the same.",
        ...
        ("Edit inventory group", "Apply") => "Save this group's name and every pending rule change, then close. An empty rule list matches nothing. No immediate transfers; maintained loadouts referencing this group will use the new rules on their next pass.",
        ("Edit inventory group", "Add rule") => "Add another alternative block/role/item rule to the draft. At most 128 rules per group. Apply in this window saves all rows.",
        ("Edit inventory group", "Edit rule") => "Edit exactly one selected rule. Save rule returns its changes to this draft; Apply commits the group.",
        ("Edit inventory group", "Duplicate") => "Copy selected rules into the draft. Copies are independent. Overlapping rules never count the same inventory or stack twice. Apply saves all changes.",
        ("Edit inventory group", "Remove") => "Remove selected rules from the draft. Removing every row makes the group match nothing. Apply commits; Cancel discards all draft changes.",
        ("Edit group rule", "Save rule") => "Keep this rule in the parent editor's draft and return to its list. The group is not saved until you press Apply there.",
        ...
        ("Edit inventory group" or "Edit group rule" or "Edit loadout", "Close" or "Cancel") => "Discard unsaved edits in this editor and close. Previously saved settings remain unchanged.",
        ...
        (_, "Profile tools") => "Open owner-only server-profile listing, binding recovery, deletion and section patching. Opening the window changes nothing.",
        (_, "List / refresh") => "Fetch this page of server profiles owned by you. Does not bind, delete or adopt a profile.",
        (_, "Previous page") => "Fetch the previous page of owned server profiles.",
        (_, "Next page") => "Fetch the next page of owned server profiles.",
